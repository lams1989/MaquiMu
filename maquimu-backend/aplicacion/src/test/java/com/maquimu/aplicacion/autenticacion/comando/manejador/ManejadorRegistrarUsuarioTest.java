...
class ManejadorRegistrarUsuarioTest {
    ...
    class HashingPassword {
        ...
        void ejecutar_deberiaHashearPasswordAntesDeGuardar() {
            ...
            when(fabricaUsuario.crear(eq(comando), eq(passwordHashEsperado))).thenReturn(
                    crearUsuarioGuardado(null, "Juan Pérez", "juan@maquimu.com", RolUsuario.OPERARIO)
            );
            when(usuarioRepositorio.guardar(any())).thenReturn(
                    crearUsuarioGuardado(1L, "Juan Pérez", "juan@maquimu.com", RolUsuario.OPERARIO)
            );
