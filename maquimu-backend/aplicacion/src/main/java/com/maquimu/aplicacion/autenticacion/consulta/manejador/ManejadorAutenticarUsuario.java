...
public class ManejadorAutenticarUsuario {
	...
	public ComandoAutenticacion ejecutar(ConsultaAutenticarUsuario consulta) {
		Authentication authentication = authenticationManager
				.authenticate(new UsernamePasswordAuthenticationToken(consulta.getEmail(), consulta.getPassword()));

		// Si la autenticación es exitosa, recupera UserDetails y genera un token.
		UserDetails userDetails = (UserDetails) authentication.getPrincipal();

		// Recuperar el objeto Usuario completo de la base de datos usando el correo
		// electrónico de UserDetails
		...
		if (usuario.getEstado() == EstadoUsuario.PENDIENTE_APROBACION) {
			throw new CuentaNoActivaException(EstadoUsuario.PENDIENTE_APROBACION,
		...
		if (usuario.getEstado() == EstadoUsuario.RECHAZADO) {
			throw new CuentaNoActivaException(EstadoUsuario.RECHAZADO,
		...
		if (usuario.getEstado() == EstadoUsuario.RESTABLECER) {
			throw new CuentaNoActivaException(EstadoUsuario.RESTABLECER,
		...
		return ComandoAutenticacion.builder().token(token).usuario(usuario).build();
	}
}
