...
public class ServicioGestionAlquiler {
    ...
    public ServicioGestionAlquiler(AlquilerRepositorio alquilerRepositorio,
     ...
     * Valida cliente, maquinaria, disponibilidad, calcula costo y persiste.
     * 
    ...
    public Alquiler solicitarAlquiler(ComandoSolicitarAlquiler comando) {
        ...
        clienteDao.buscarPorId(comando.getClienteId())
        ...
        Maquinaria maquinaria = maquinariaDao.buscarPorId(comando.getMaquinariaId())
                .orElseThrow(() -> new IllegalArgumentException("Maquinaria no encontrada con ID: " + comando.getMaquinariaId()));

        // 3. Validar disponibilidad de la maquinaria en el rango de fechas
        boolean disponible = validadorDisponibilidad.estaDisponible(
                comando.getMaquinariaId(),
                comando.getFechaInicio(),
                comando.getFechaFin()
        );
        ...
        BigDecimal costoTotal = calculadorCosto.calcularCostoPorDias(
                maquinaria,
                comando.getFechaInicio(),
                comando.getFechaFin()
        );
