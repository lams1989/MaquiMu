package com.maquimu.infraestructura.factura.adaptador.dao;
...
import com.maquimu.infraestructura.factura.adaptador.repositorio.JpaFacturaRepository;
...
public class FacturaDaoJpa implements FacturaDao {
	...
	public List<Factura> listarTodas() {
		return jpaFacturaRepository.findAll().stream().map(FacturaEntity::toFactura).collect(Collectors.toList());
	}

	@Override
	public Optional<Factura> buscarPorId(Long facturaId) {
		return jpaFacturaRepository.findById(facturaId).map(FacturaEntity::toFactura);
	}

	@Override
	public Optional<Factura> buscarPorAlquilerId(Long alquilerId) {
		return jpaFacturaRepository.findByAlquiler_AlquilerId(alquilerId).map(FacturaEntity::toFactura);
	}

	@Override
	public List<Factura> buscarPorClienteId(Long clienteId) {
		return jpaFacturaRepository.findByCliente_ClienteId(clienteId).stream().map(FacturaEntity::toFactura)
				.collect(Collectors.toList());
	}

	@Override
	public List<Factura> listarPorEstadoPago(EstadoPago estadoPago) {
		return jpaFacturaRepository.findByEstadoPago(estadoPago).stream().map(FacturaEntity::toFactura)
				.collect(Collectors.toList());
	}
