...
public class SeguridadConfig {
	...
	public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		http.csrf(csrf -> csrf.disable()).cors(cors -> cors.configurationSource(corsConfigurationSource()))
				...
				.authorizeHttpRequests(auth -> auth.requestMatchers("/api/maquimu/v1/auth/**").permitAll()
						.requestMatchers("/actuator/health").permitAll().requestMatchers("/error").permitAll()
		...
		for (String origin : origins) {
			...
			if ("*".equals(trimmed)) {
				...
				if (configuration.getAllowedOrigins() == null) {
					configuration.setAllowedOrigins(new java.util.ArrayList<>());
				}
