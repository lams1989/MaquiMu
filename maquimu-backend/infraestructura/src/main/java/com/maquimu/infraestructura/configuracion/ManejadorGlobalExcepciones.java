...
public class ManejadorGlobalExcepciones {
	...
	public ResponseEntity<Map<String, Object>> manejarDataIntegrityViolation(DataIntegrityViolationException ex) {
		String mensaje = esViolacionUnicidad(ex) ? "Ya existe un registro con la informaci\u00f3n proporcionada"
	...
	private boolean esViolacionUnicidad(Throwable throwable) {
		...
		while (causaActual != null) {
			...
			if (mensaje != null) {
				...
				if (mensajeNormalizado.contains("duplicate") || mensajeNormalizado.contains("duplicado")
						|| mensajeNormalizado.contains("unique") || mensajeNormalizado.contains("uniq")) {
					return true;
				}
			}
