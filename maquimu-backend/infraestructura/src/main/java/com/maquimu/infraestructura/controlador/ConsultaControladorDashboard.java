...
import com.maquimu.aplicacion.dashboard.DashboardClienteDTO;
import com.maquimu.aplicacion.dashboard.DashboardOperarioDTO;
import com.maquimu.aplicacion.dashboard.ManejadorDashboardCliente;
import com.maquimu.aplicacion.dashboard.ManejadorDashboardOperario;
...
public class ConsultaControladorDashboard {
    ...
    public ConsultaControladorDashboard(ManejadorDashboardOperario manejadorDashboardOperario,
                                        ManejadorDashboardCliente manejadorDashboardCliente,
    ...
    /**
     * Obtiene los KPIs del dashboard del operario.
     * GET /api/maquimu/v1/dashboard/operario
     */
    @GetMapping("/operario")
    public ResponseEntity<DashboardOperarioDTO> obtenerDashboardOperario() {
        DashboardOperarioDTO dashboard = manejadorDashboardOperario.ejecutar();
        return ResponseEntity.ok(dashboard);
    }

    /**
     * Obtiene los KPIs del dashboard del cliente autenticado.
     * GET /api/maquimu/v1/dashboard/cliente
     */
    @GetMapping("/cliente")
    public ResponseEntity<DashboardClienteDTO> obtenerDashboardCliente(HttpServletRequest request) {
        Cliente cliente = extraerClienteDesdeJwt(request);
        DashboardClienteDTO dashboard = manejadorDashboardCliente.ejecutar(cliente.getClienteId());
        return ResponseEntity.ok(dashboard);
    }
