...
/**
 * Manejador del comando para generar una factura.
 * Reglas de negocio:
 * - El alquiler debe existir y estar en estado FINALIZADO.
 * - No puede existir una factura previa para el mismo alquiler.
 */
@Service
    ...
    public Factura ejecutar(ComandoGenerarFactura comando) {
        ...
        Alquiler alquiler = alquilerDao.buscarPorId(comando.getAlquilerId())
                .orElseThrow(() -> new IllegalArgumentException(
                        "Alquiler no encontrado con ID: " + comando.getAlquilerId()));

        // 2. Validar que el alquiler esté FINALIZADO
        if (alquiler.getEstado() != EstadoAlquiler.FINALIZADO) {
            throw new IllegalStateException(
                    "Solo se puede generar factura para alquileres en estado FINALIZADO. " +
