package com.maquimu.infraestructura.cliente.adaptador.dao;
...
public class JpaClienteDao implements ClienteDao {
	...
	public List<Cliente> listarTodos() {
		return jpaClienteRepository.findAll().stream().map(ClienteEntity::toCliente).collect(Collectors.toList());
	}

	@Override
	public Optional<Cliente> buscarPorId(Long id) {
		return jpaClienteRepository.findById(id).map(ClienteEntity::toCliente);
	}

	@Override
	public Optional<Cliente> buscarPorIdentificacion(String identificacion) {
		return jpaClienteRepository.findByIdentificacion(identificacion).map(ClienteEntity::toCliente);
	}

	@Override
	public Optional<Cliente> buscarPorEmail(String email) {
		return jpaClienteRepository.findByEmail(email).map(ClienteEntity::toCliente);
	}

	@Override
	public Optional<Cliente> buscarPorUsuarioId(Long usuarioId) {
		return jpaClienteRepository.findByUsuarioId(usuarioId).map(ClienteEntity::toCliente);
	}
	...
	public boolean existePorEmail(String email) {
		return jpaClienteRepository.existsByEmail(email);
	}
}
